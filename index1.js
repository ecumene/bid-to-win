...
function newUser(){
    ...
    if(user.length > 3 && user.length < 17 && key.length > 3 && key.length < 17){
        const baseURL = 'http://localhost:3000/user/1.0.0/create';
        fetch(baseURL, {
            method: 'POST',
            headers: {
...
function userStats(){
    ...
    baseURL2 = `http://localhost:3000/user_stats/1.0.0/gprank/:Username?Username=${user}`
    fetch(baseURL2)
        .then(response => response.json())
        .then(data => {
            if(data.length == 0){
    ...
    baseURL3 = `http://localhost:3000/user_stats/1.0.0/winsrank/:Username?Username=${user}`
    fetch(baseURL3)
        .then(response => response.json())
        .then(data => {
            if(data.length == 0){
    ...
    baseURL4 = `http://localhost:3000/user_stats/1.0.0/winperrank/:Username?Username=${user}`
    fetch(baseURL4)
        .then(response => response.json())
        .then(data => {
            if(data.length == 0){
...
function leaderboard(){
    ...
    const baseURL = 'http://localhost:3000/1.0.0/leaderboard';
    fetch(baseURL)
        .then(response => response.json())
        .then(data => {
...
function trickGen(){
    ...
    if(cpu == 1 && youradj.length == 0 && user != null){
        gp++
        abs++
        const baseURL = 'http://localhost:3000/user/1.0.0/game_started';
        fetch(baseURL, {
            method: 'PUT',
            headers: {
                'Accept': 'application/json',
...
function scoreReveal(){
    ...
    if (scorerev >= 4 && youradj.length != 10){    
        ...
        if(cpu == 1 && youradj.length == 10 && yourscore > oppscore && user != null){
            ...
            const baseURL = 'http://localhost:3000/user/1.0.0/win';
            fetch(baseURL, {
                method: 'PUT',
                headers: {
                    'Accept': 'application/json',
            ...
            abs--
            winper = Math.round((wins * 100)/(wins + losses));
            const baseURL = 'http://localhost:3000/user/1.0.0/loss';
            fetch(baseURL, {
                method: 'PUT',
                headers: {
                    'Accept': 'application/json',
            ...
            const baseURL = 'http://localhost:3000/user/1.0.0/tie';
            fetch(baseURL, {
                method: 'PUT',
                headers: {
                    'Accept': 'application/json',
